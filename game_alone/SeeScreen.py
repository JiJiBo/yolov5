...
class SeeScreen:
    def __init__(self, config):
        ...
        self.mouse = MouseUtils(self.config.instance.ads)
    ...
    def start_monitoring(self):
        """实时监视屏幕中央区域"""
        delay = 1 / self.fps
        while not self.config.instance.isDes:
            start_time = time.time()
            screenshot = self.capture_center_area()
            frame = cv2.cvtColor(np.array(screenshot), cv2.COLOR_BGR2RGB)
            print(self.config.instance.isStarted)
            if self.config.instance.isStarted:
